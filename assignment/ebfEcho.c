...
int main(int argc, char **argv)
{
    ...
    for (int current = 0; current < numBytes; current++)
        { // writing out
        if (imageData[current] < 0 || imageData[current] > 31) {
            free(imageData);
            fclose(outputFile);
            printf("ERROR: Bad Data (%s)\n", argv[1]);
            return BAD_DATA;
        ...
        check = fprintf(outputFile, "%u", imageData[current]);
        if (check == 0)
            { // check write
            fclose(outputFile);
            free(imageData);
        ...
        if (current + 1 == numBytes) break;
        check = fprintf(outputFile, "%c", ((current + 1) % width) ? ' ' : '\n');
        if (check == 0)
            { // check write
            fclose(outputFile);
            free(imageData);
